# Dial positions for /design-expressive
...
- Color blocks as architecture: full-bleed section backgrounds that flip the scheme (light -> dark -> accent-flooded). The flip is the rhythm of the page.
...
- Build a strict 12-col grid, then violate it deliberately: headlines spanning off-canvas, images overlapping two sections, a paragraph starting at column 7, elements at slight rotation (-2 to 3 degrees), stacked z-layers with the text on top.
- Asymmetry everywhere; whitespace in unexpected places (a near-empty viewport-height pause between dense sections is a power move).
...
- Entrances with conviction: headlines reveal per-line with clip-path or translate+fade, 500-700ms, easing `cubic-bezier(0.16, 1, 0.3, 1)`, staggered 60-80ms.
- Scroll-driven moments: subtle parallax (0.85-1.15 speed factors), images scaling 1.1 -> 1 as they enter; the one big idea often lives here.
- Hover states that delight: images tilt/zoom, links with sliding underlines or word-swap effects, cursor changes over interactive zones.
...
The big idea executed timidly (an 80px headline is just a headline; 160px is a statement) · ten effects at 70% quality instead of three at 100% · gradient-text-on-purple (loud is not that) · sacrificing body readability or a11y for the show · motion that blocks reading or input · breaking the grid before establishing it.
